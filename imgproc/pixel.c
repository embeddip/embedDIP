 ...
 *
 * This function computes the negative of a given image by subtracting each
 * pixel value from the maximum intensity value (typically 255 for 8-bit
 * grayscale images).
 *
 * @param[in]  inImg   Pointer to the input image structure (source image).
 * @param[out] outImg  Pointer to the output image structure (resulting negative
 * image).
 *
 * @note Assumes both images are of the same size and format, and `UINT8_MAX` is
 ...
 *
 * This function sets each pixel to 255 if its value is greater than or equal to
 * the threshold, and to 0 otherwise. The output image will be a binary image
 * (black and white).
 *
 * @param[in]  inImg      Pointer to the input image structure (grayscale
 * image).
 * @param[out] outImg     Pointer to the output image structure (thresholded
 * image).
...
void grayscaleThresholdLocalOtsu(const Image *inImg, Image *outImg,
                                 int blockSize) {
 ...
 *
 * This function applies K-means clustering to segment a grayscale image into
 * `k` clusters. Each pixel in the output image is assigned the cluster center
 * value it belongs to.
 *
 ...
 *
 * This function applies K-means clustering to an HSI image by treating each
 * pixel as a 3D vector (Hue, Saturation, Intensity). It assigns each pixel to a
 * cluster and sets the output to the corresponding cluster center color.
 *
...
static inline float vec_distance_native(const float a[3], const float b[3],
                                        ImageFormat fmt) {
 ...
 * Each pixel is replaced by its cluster center color (in the same
 * space/format).
 *
...
void colorKMeans3(const Image *inImg, Image *outImg, int k) {
  ...
  float *centers =
      (float *)memory_alloc((size_t)k * 3 * sizeof(float)); /* k x 3 */
  float *sums =
      (float *)memory_alloc((size_t)k * 3 * sizeof(float)); /* k x 3 */
...
void grayscaleKMeans(const Image *inImg, Image *outImg, int k)
{
    ...
    float *M = (float *)memory_alloc(k * sizeof(float));   // Cluster means M_1
to M_K float *S = (float *)memory_alloc(k * sizeof(float));   // Cluster sums
S_k int *T = (int *)memory_alloc(k * sizeof(int));         // Cluster sample
counts T_k int *C = (int *)memory_alloc(N * sizeof(int));         // Cluster
...
/*
void grayscaleRegionGrowing(const Image *inImg, Image *outImg, int seedX, int
seedY, uint8_t tolerance)
{
    const int width = inImg->width;
    const int height = inImg->height;
...
/**
 * @brief Segments a grayscale image using region growing with multiple seed
 * points.
 *
 ...
 *
 * @param[in]  inImg      Pointer to input grayscale image.
 * @param[out] outImg     Pointer to output binary image (0 for background, 255
 * for region).
 ...
 */
void grayscaleRegionGrowing(const Image *inImg, Image *outImg,
                            const Point *seeds, int numSeeds,
                            uint8_t tolerance) {
  const int width = inImg->width;
  const int height = inImg->height;
  const uint8_t *src = inImg->pixels;
 ...
 *
 * Grows a region from a seed point by comparing HSI vectors using Euclidean
 * distance. Pixels within a given threshold are included in the region.
 *
 ...
 */
void colorRegionGrowing(const Image *inImg, Image *outImg, int seedX, int seedY,
                        float tolerance) {
  assert(inImg && outImg);
  assert(inImg->format == IMAGE_FORMAT_HSI);
  assert(outImg->format == IMAGE_FORMAT_HSI);
...
static inline float color_distance(const float a[3], const float b[3],
                                   ImageFormat fmt) {
  if (fmt == IMAGE_FORMAT_HSI) {
 ...
 *
 * The output image must be single-channel grayscale (IMAGE_FORMAT_GRAYSCALE,
 * depth=1).
 *
 ...
 */
void colorRegionGrowing3(const Image *inImg, Image *outImg, const Point *seeds,
                         int numSeeds, float tolerance) {
  assert(inImg && outImg);
  ...
  for (int s = 0; s < numSeeds; ++s) {
    ...
    if ((unsigned)seedX >= (unsigned)width ||
        (unsigned)seedY >= (unsigned)height)
      continue;
...
/**
 * @brief Detects straight lines in a binary edge image using the Hough
 * Transform.
 *
 * This function computes the Hough accumulator space for all edge pixels in the
 * input image. Each edge pixel votes for all lines passing through it,
 * parameterized by (rho, theta).
 *
 * @param[in]  edgeImg     Pointer to the binary edge image (output of
 * Sobel/Canny).
 ...
 */
void houghTransform(const Image *edgeImg, int **accumulator, int numRho,
                    int numTheta, float rhoRes, float thetaRes) {
...
int extractLines(int **accumulator, int numRho, int numTheta, float rhoRes,
                 float thetaRes, int threshold, float rhoMax, HoughLine *lines,
                 int maxLines) {
  int count = 0;
 ...
 */
void drawLine(Image *img, uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1,
              uint8_t color) {
...
void drawLineOnImage(Image *img, float rho, float theta, uint8_t color) {
  ...
  drawLine(img, x1, y1, x2, y2,
           color); // your custom Bresenham-style line drawer
}
...
void connectedComponents(const Image *inImg, Image *outImg) {
  ...
  Image *equivalences =
      (Image *)createImage(IMAGE_RES_WQVGA, IMAGE_FORMAT_GRAYSCALE);
 ...
 *
 * Erosion removes pixels on object boundaries. A pixel in the output image is
 * set to 255 only if all corresponding pixels under the non-zero elements of
 * the kernel are also non-zero in the input image. This process can be repeated
 * for multiple iterations to achieve a stronger erosion effect.
 *
 * @param[in]  src         Pointer to the source binary image.
 * @param[out] dst         Pointer to the destination image to store the eroded
 * result.
 * @param[in]  kernel      Pointer to the structuring element (Kernel) used for
 * erosion.
 * @param[in]  iterations  Number of times erosion is applied.
 */
void erode(const Image *src, Image *dst, const Kernel *kernel,
           uint8_t iterations) {
 ...
 * to 255 if at least one corresponding pixel under the non-zero elements of the
 * kernel is non-zero in the input image. This process can be repeated for
 * multiple iterations to achieve a stronger dilation effect.
 *
 * @param[in]  src         Pointer to the source binary image.
 * @param[out] dst         Pointer to the destination image to store the dilated
 * result.
 * @param[in]  kernel      Pointer to the structuring element (Kernel) used for
 * dilation.
 * @param[in]  iterations  Number of times dilation is applied.
 */
void dilate(const Image *src, Image *dst, const Kernel *kernel,
            uint8_t iterations) {
  int w = src->width, h = src->height;
  int kSize = kernel->size;
  int anchor = kernel->anchor;
 ...
 * preserving the overall shape and size of larger objects in the image.
 *
 * @param[in]  inImg       Pointer to the input binary image.
 * @param[out] outImg      Pointer to the output image to store the result of
 * opening.
 * @param[in]  kernel      Pointer to the structuring element (Kernel) used for
 * the operation.
 * @param[in]  iterations  Number of erosion/dilation iterations.
 */
void opening(const Image *inImg, Image *outImg, const Kernel *kernel,
             uint8_t iterations) {
 ...
 * element. It is typically used to fill small holes and gaps in the objects
 * while preserving their general shape.
 *
 * @param[in]  inImg       Pointer to the input binary image.
 * @param[out] outImg      Pointer to the output image to store the result of
 * closing.
 ...
 */
void closing(const Image *inImg, Image *outImg, const Kernel *kernel,
             uint8_t iterations) {
  Image *temp = createImage(IMAGE_RES_WQVGA, IMAGE_FORMAT_GRAYSCALE);
...
/**
 * @brief Applies a power-law (gamma) transformation and stores the result in
 * float channels.
 *
 ...
 * @param[out] outImg  Pointer to the output image. Allocated `chals` field will
 * be filled.
...
/**
 * @brief Applies linear scaling and bias followed by absolute value and clamps
 * to 8-bit.
 *
 * This function simulates OpenCV's convertScaleAbs: dst =
 * saturate_cast<uchar>(abs(alpha * src + beta))
 *
 * @param[in]  inImg   Pointer to the input image (supports GRAYSCALE or RGB,
 * 8-bit).
 * @param[out] outImg  Pointer to the output image. Output must have pixels
 * allocated.
 * @param[in]  alpha   Gain factor.
 * @param[in]  beta    Bias added after scaling.
 */
void convertScaleAbs(const Image *inImg, Image *outImg, float alpha,
                     float beta) {
 ...
 * @param[in]  values       Corresponding output values (in [0, 1] or desired
 * float range).
...
void _xor(const Image *img1, const Image *img2, Image *outImg) {
  if (!img1 || !img2 || !outImg || img1->width != img2->width ||
      img1->height != img2->height || img1->log != IMAGE_DATA_PIXELS ||
...
/**
 * @brief Performs a simplified GrabCut-inspired segmentation on a grayscale
 * image.
 *
 * It refines a binary mask using iterative classification based on
 * foreground/background means.
 *
 * @param[in]  inImg     Input grayscale image.
 * @param[in,out] mask   Trimap mask: 0 = background, 1 = probable, 2 =
 * foreground.
...
/**
 * @brief Performs a simplified GrabCut-inspired segmentation on a grayscale
 * image using a rectangular ROI.
 *
 * All pixels inside the ROI are considered probable foreground and refined over
 * several iterations. Pixels outside the ROI are background.
 *
 * @param[in]  inImg      Input grayscale image.
 * @param[out] outImg     Output binary image (0 = background, 255 =
 * foreground).
 * @param[in]  roi        Rectangleangular region of interest.
 * @param[in]  iterations Number of refinement iterations.
 */
void grabCutLite(Image *inImg, Image *outImg, Rectangle roi, int iterations) {
  if (!inImg || !outImg || !inImg->pixels || !outImg->pixels) {
    printf("grabCutLite: Null image or pixel buffer\n");
    return;
  }
  ...
  if (width <= 0 || height <= 0 || size <= 0) {
    printf("grabCutLite: Invalid image dimensions\n");
    return;
  }

  if (iterations <= 0) {
    printf("grabCutLite: Iterations must be > 0\n");
    return;
  }
  ...
  // Allocate temporary mask (0 = BG, 1 = probable, 2 = FG)
  uint8_t *mask = (uint8_t *)memory_alloc(size);
  if (!mask) {
    printf("grabCutLite: Memory allocation for mask failed\n");
    return;
  }
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
      int idx = y * width + x;
      if (x >= roi.x && x < (roi.x + roi.width) && y >= roi.y &&
          y < (roi.y + roi.height)) {
        mask[idx] = 1; // probable
      } else {
  ...
  for (int iter = 0; iter < iterations; ++iter) {
    ...
    // Bootstrap foreground if no confirmed FG yet
    if (fgCount == 0) {
      for (int i = 0; i < size; ++i) {
        if (mask[i] == 1) {
          fgSum += src[i];
    ...
    if (fgCount == 0 || bgCount == 0) {
      printf(
          "grabCutLite: Not enough FG/BG samples, stopping early (iter %d)\n",
          iter);
      break;
    }

    const uint8_t fgMean = (uint8_t)(fgSum / fgCount);
    const uint8_t bgMean = (uint8_t)(bgSum / bgCount);

    // Reset ROI to "probable"
  ...
  // Mark the output as valid pixel data
  outImg->log = IMAGE_DATA_PIXELS;
}
...
static float gaussian_prob(float x, float mean, float var) {
  float diff = x - mean;
  return (1.0f / sqrtf(2.0f * M_PI * var)) *
         expf(-(diff * diff) / (2.0f * var));
}

void grabCutGrayscaleRealistic(const Image *inImg, Image *outMask,
                               Rectangle roi, int max_iter) {
  if (!inImg || !outMask || !inImg->pixels ||
  ...
  uint8_t *labels =
      (uint8_t *)memory_alloc(size * sizeof(uint8_t)); // 0=BG, 1=FG
  float (*fg_resp)[GMM_COMPONENTS] = (float (*)[GMM_COMPONENTS])memory_alloc(
      size * GMM_COMPONENTS * sizeof(float));
  float (*bg_resp)[GMM_COMPONENTS] = (float (*)[GMM_COMPONENTS])memory_alloc(
      size * GMM_COMPONENTS * sizeof(float));
  ...
  for (int iter = 0; iter < max_iter; ++iter) {
    ...
    for (int i = 0; i < size; ++i) {
      ...
      for (int c = 0; c < GMM_COMPONENTS; ++c) {
        fg_resp[i][c] = fg_gmm[c].weight *
                        gaussian_prob(x, fg_gmm[c].mean, fg_gmm[c].variance);
      ...
      for (int c = 0; c < GMM_COMPONENTS; ++c) {
        bg_resp[i][c] = bg_gmm[c].weight *
                        gaussian_prob(x, bg_gmm[c].mean, bg_gmm[c].variance);
    ...
    for (int i = 0; i < size; ++i) {
      ...
      for (int c = 0; c < GMM_COMPONENTS; ++c) {
        p_fg += fg_gmm[c].weight *
                gaussian_prob(x, fg_gmm[c].mean, fg_gmm[c].variance);
        p_bg += bg_gmm[c].weight *
                gaussian_prob(x, bg_gmm[c].mean, bg_gmm[c].variance);
      }
...
float gaussian_prob_rgb(const uint8_t *pixel, const GMMComponentRGB *comp) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    prob *=
        (1.0f / sqrtf(2.0f * M_PI * var)) * expf(-diff * diff / (2.0f * var));
  }
  return prob;
}

void grabCutRGB(const Image *inImg, Image *outMask, Rectangle roi,
                int max_iter) {
  if (!inImg || !outMask || !inImg->pixels ||
      inImg->format != IMAGE_FORMAT_RGB888)
    return;
    ...
    for (int c = 0; c < GMM_COMPONENTS; ++c) {
      ...
      for (int ch = 0; ch < 3; ++ch) {
        if (fg_wsum > 1e-6f) {
          ...
          float var = (fg_sqsum[ch] / fg_wsum) -
                      fg_gmm[c].mean[ch] * fg_gmm[c].mean[ch];
          fg_gmm[c].variance[ch] = fmaxf(var, 10.0f);
        }

        if (bg_wsum > 1e-6f) {
          bg_gmm[c].mean[ch] = bg_sum[ch] / bg_wsum;
          float var = (bg_sqsum[ch] / bg_wsum) -
                      bg_gmm[c].mean[ch] * bg_gmm[c].mean[ch];
  ...
  for (int y = 0; y < outHeight; y++) {
    for (int x = 0; x < outWidth; x++) {
      ...
      outImg->chals->ch[0][y * outWidth + x] =
          (float)((uint8_t *)
                      inImg->pixels)[nearest_y * inImg->width + nearest_x];
    }
  }
 ...
 *
 * This function adds the corresponding float values from `img1` and `img2`, and
 * stores the result in `outImg->chals->ch[0][i]`.
 *
 * Assumes all images have the same resolution and float output channel
 * allocated.
 *
...
void dist(const Image *inImg, Image *outImg, uint8_t R_ref, uint8_t G_ref,
          uint8_t B_ref)
/**
 * @brief Computes the color distance of each pixel in an RGB
 * image to a given reference color.
 *
 * @param[in] inImg Pointer to the input RGB image (3 channels, interleaved as
 * RGBRGB...).
 * @param[out] outImg Pointer to the output grayscale image (1 channel, same
 * width and height as input).
...
void convertTo(Image *inImg) {
  ...
  case IMAGE_FORMAT_HSI: {
    ...
    for (uint32_t i = 0; i < inImg->size; i++) {
      data[i * 3 + 0] =
          normalize_to_u8(inImg->chals->ch[ch_b][i], min_b, max_b);
      data[i * 3 + 1] =
          normalize_to_u8(inImg->chals->ch[ch_g][i], min_g, max_g);
      data[i * 3 + 2] =
          normalize_to_u8(inImg->chals->ch[ch_r][i], min_r, max_r);
    }
    break;
  }
