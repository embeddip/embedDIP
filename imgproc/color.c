...
/**
 * @brief Applies hue-based filtering to an HSI image.
 *
 * This function retains pixels whose hue lies within the given range.
 * Pixels outside the range are set to black (H=0, S=0, I=0).
 *
 * @param[in]  input     Pointer to the input HSI image.
 * @param[out] output    Pointer to the output HSI image.
...
void hueThreshold(const Image *input, Image *output, float minHue, float maxHue)
{
    ...
    for (uint32_t i = 0; i < input->size; ++i)
    {
        ...
        uint8_t s = in[i * 3 + 1]; // Saturation
        uint8_t v = in[i * 3 + 2]; // Intensity
