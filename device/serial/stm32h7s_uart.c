...
static int serial_capture(Image *img)
{
    ...
    HAL_UART_Transmit(&huart4, request_start_sequence, 3, 5000);
    ...
    HAL_UART_Transmit(&huart4, (uint8_t *)(&width_u32), sizeof(uint32_t), 1000);
    HAL_UART_Transmit(&huart4, (uint8_t *)(&height_u32), sizeof(uint32_t), 1000);
    HAL_UART_Transmit(&huart4, (uint8_t *)(&format_u32), sizeof(uint32_t), 1000);
    HAL_UART_Transmit(&huart4, (uint8_t *)(&depth_u32), sizeof(uint32_t), 1000);
    ...
    for (uint32_t i = 0; i < blockCount; i++) {
        status =
            HAL_UART_Receive(&huart4, pixelPtr, blockSize, 10000);  // 10 second timeout per block
        if (status != HAL_OK) {
    ...
    if (lastBlockSize > 0) {
        status = HAL_UART_Receive(&huart4, pixelPtr, lastBlockSize, 10000);
        if (status != HAL_OK) {
            __HAL_UART_CLEAR_FLAG(&huart4, UART_CLEAR_OREF | UART_CLEAR_NEF | UART_CLEAR_FEF);
            return EMBEDDIP_ERROR_IO_ERROR;
        }
...
static int serial_send(const Image *img)
{
    ...
    HAL_UART_Transmit(&huart4, request_start_sequence, 3, HAL_MAX_DELAY);
    ...
    HAL_UART_Transmit(&huart4, (uint8_t *)(&width_u32), sizeof(uint32_t), HAL_MAX_DELAY);
    HAL_UART_Transmit(&huart4, (uint8_t *)(&height_u32), sizeof(uint32_t), HAL_MAX_DELAY);
    HAL_UART_Transmit(&huart4, (uint8_t *)(&format_u32), sizeof(uint32_t), HAL_MAX_DELAY);
    HAL_UART_Transmit(&huart4, (uint8_t *)(&depth_u32), sizeof(uint32_t), HAL_MAX_DELAY);
    HAL_Delay(2);  // Allow receiver to process metadata

    // Step 3: Send image pixel data in blocks
    const uint8_t *pixelPtr = img->pixels;
    for (uint32_t i = 0; i < blockCount; i++) {
        HAL_UART_Transmit(&huart4, pixelPtr, blockSize, HAL_MAX_DELAY);
        pixelPtr += blockSize;
    ...
    if (lastBlockSize > 0) {
        HAL_UART_Transmit(&huart4, pixelPtr, lastBlockSize, HAL_MAX_DELAY);
        HAL_Delay(1);  // <-- 1-2ms delay between blocks
    }
    return EMBEDDIP_OK;
}
...
static int serial_send_jpeg(const Image *img)
{
    ...
    HAL_UART_Transmit(&huart4, (uint8_t *)header, sizeof(header), HAL_MAX_DELAY);

    // Send JPEG size (uint32_t)
    HAL_UART_Transmit(&huart4, (uint8_t *)&img->size, sizeof(img->size), HAL_MAX_DELAY);
    ...
    while (remaining > 0) {
        ...
        HAL_UART_Transmit(&huart4, ptr, chunk, HAL_MAX_DELAY);
        ptr += chunk;
        remaining -= chunk;
    }
    return EMBEDDIP_OK;
...
serial_send_1d(const void *data, uint8_t elem_size, uint32_t length, Serial1DDataType type)
{
    ...
    HAL_UART_Transmit(&huart4, (uint8_t *)&length, sizeof(length), HAL_MAX_DELAY);
    HAL_UART_Transmit(&huart4, &elem_size, sizeof(elem_size), HAL_MAX_DELAY);
    HAL_Delay(1);
