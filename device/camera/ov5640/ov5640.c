...
/**
 * @brief  Set the OV5640 camera pixel format.
 * @param  DeviceAddr: Device address on communication Bus.
 * @param  PixelFormat: Pixel format to be configured (OV5640_PIXEL_FORMAT_xxx)
 * @retval 0 if success, -1 if error
 */
int32_t OV5640_SetPixelFormat(uint16_t DeviceAddr, uint32_t PixelFormat)
{
  uint32_t index;

  /* Pixel format register arrays based on real driver */
  static const uint16_t OV5640_PF_RGB565[][2] = {
      {0x4300, 0x6F}, /* FORMAT_CTRL00: RGB565 */
      {0x501F, 0x01}, /* FORMAT_MUX_CTRL: enable format */
  };

  static const uint16_t OV5640_PF_YUV422[][2] = {
      {0x4300, 0x30}, /* FORMAT_CTRL00: YUV422 */
      {0x501F, 0x00}, /* FORMAT_MUX_CTRL */
  };

  static const uint16_t OV5640_PF_RGB888[][2] = {
      {0x4300, 0x23}, /* FORMAT_CTRL00: RGB888 */
      {0x501F, 0x01}, /* FORMAT_MUX_CTRL: enable format */
  };

  static const uint16_t OV5640_PF_Y8[][2] = {
      {0x4300, 0x10}, /* FORMAT_CTRL00: Y8 (grayscale) */
      {0x501F, 0x00}, /* FORMAT_MUX_CTRL */
  };

  static const uint16_t OV5640_PF_JPEG[][2] = {
      {0x4300, 0x30}, /* FORMAT_CTRL00: JPEG */
      {0x501F, 0x00}, /* FORMAT_MUX_CTRL */
  };
  ...
  switch (PixelFormat)
  {
  case OV5640_PIXEL_FORMAT_RGB565:
    for (index = 0; index < (sizeof(OV5640_PF_RGB565) / sizeof(OV5640_PF_RGB565[0])); index++)
    {
      CAMERA_IO_Write(DeviceAddr, OV5640_PF_RGB565[index][0], OV5640_PF_RGB565[index][1]);
    }
    break;

  case OV5640_PIXEL_FORMAT_YUV422:
    for (index = 0; index < (sizeof(OV5640_PF_YUV422) / sizeof(OV5640_PF_YUV422[0])); index++)
    {
      CAMERA_IO_Write(DeviceAddr, OV5640_PF_YUV422[index][0], OV5640_PF_YUV422[index][1]);
    }
    break;

  case OV5640_PIXEL_FORMAT_RGB888:
    for (index = 0; index < (sizeof(OV5640_PF_RGB888) / sizeof(OV5640_PF_RGB888[0])); index++)
    {
      CAMERA_IO_Write(DeviceAddr, OV5640_PF_RGB888[index][0], OV5640_PF_RGB888[index][1]);
    }
    break;

  case OV5640_PIXEL_FORMAT_Y8:
    for (index = 0; index < (sizeof(OV5640_PF_Y8) / sizeof(OV5640_PF_Y8[0])); index++)
    {
      CAMERA_IO_Write(DeviceAddr, OV5640_PF_Y8[index][0], OV5640_PF_Y8[index][1]);
    }
    break;

  case OV5640_PIXEL_FORMAT_JPEG:
    for (index = 0; index < (sizeof(OV5640_PF_JPEG) / sizeof(OV5640_PF_JPEG[0])); index++)
    {
      CAMERA_IO_Write(DeviceAddr, OV5640_PF_JPEG[index][0], OV5640_PF_JPEG[index][1]);
    }
    /* Enable JPEG mode - set bit 5 in TIMING_TC_REG21 */
    {
      uint16_t tmp = CAMERA_IO_Read(DeviceAddr, 0x3821);
      CAMERA_IO_Write(DeviceAddr, 0x3821, tmp | (1 << 5));
    }
    /* Enable JPEG clock and disable scaling engine */
    {
      uint16_t tmp = CAMERA_IO_Read(DeviceAddr, 0x3002);
      CAMERA_IO_Write(DeviceAddr, 0x3002, tmp & ~((1 << 4) | (1 << 3) | (1 << 2)));
    }
    {
      uint16_t tmp = CAMERA_IO_Read(DeviceAddr, 0x3006);
      CAMERA_IO_Write(DeviceAddr, 0x3006, tmp | ((1 << 5) | (1 << 3)));
    }
    break;
  ...
  CAMERA_Delay(10); /* Small delay for registers to take effect */
  return 0;
}
...
/**
 * @brief  Set OV5640 camera resolution.
 * @param  DeviceAddr: Device address on communication Bus.
 * @param  Resolution: Camera resolution to set (CAMERA_R160x120, CAMERA_R320x240, etc.)
 * @retval 0 if success, -1 if error
 * @note   Only changes the output size registers. Sensor is already initialized with 800x480 base.
 */
int32_t OV5640_SetResolution(uint16_t DeviceAddr, uint32_t Resolution)
{
  uint32_t index;

  /* Resolution-specific register arrays based on real driver */
  /* Only output size needs to be changed - sensor maintains 800x480 base */

  static const uint16_t OV5640_RES_QQVGA[][2] = {  /* 160x120 */
      {0x3808, 0x00}, /* TIMING_DVPHO_HIGH */
      {0x3809, 0xA0}, /* TIMING_DVPHO_LOW = 0x00A0 = 160 */
      {0x380A, 0x00}, /* TIMING_DVPVO_HIGH */
      {0x380B, 0x78}, /* TIMING_DVPVO_LOW = 0x0078 = 120 */
  };

  static const uint16_t OV5640_RES_QVGA[][2] = {  /* 320x240 */
      {0x3808, 0x01}, /* TIMING_DVPHO_HIGH */
      {0x3809, 0x40}, /* TIMING_DVPHO_LOW = 0x0140 = 320 */
      {0x380A, 0x00}, /* TIMING_DVPVO_HIGH */
      {0x380B, 0xF0}, /* TIMING_DVPVO_LOW = 0x00F0 = 240 */
  };

  static const uint16_t OV5640_RES_480x272[][2] = {  /* 480x272 */
      {0x3808, 0x01}, /* TIMING_DVPHO_HIGH */
      {0x3809, 0xE0}, /* TIMING_DVPHO_LOW = 0x01E0 = 480 */
      {0x380A, 0x01}, /* TIMING_DVPVO_HIGH */
      {0x380B, 0x10}, /* TIMING_DVPVO_LOW = 0x0110 = 272 */
  };

  static const uint16_t OV5640_RES_VGA[][2] = {  /* 640x480 */
      {0x3808, 0x02}, /* TIMING_DVPHO_HIGH */
      {0x3809, 0x80}, /* TIMING_DVPHO_LOW = 0x0280 = 640 */
      {0x380A, 0x01}, /* TIMING_DVPVO_HIGH */
      {0x380B, 0xE0}, /* TIMING_DVPVO_LOW = 0x01E0 = 480 */
  };

  static const uint16_t OV5640_RES_WVGA[][2] = {  /* 800x480 */
      {0x3808, 0x03}, /* TIMING_DVPHO_HIGH */
      {0x3809, 0x20}, /* TIMING_DVPHO_LOW = 0x0320 = 800 */
      {0x380A, 0x01}, /* TIMING_DVPVO_HIGH */
      {0x380B, 0xE0}, /* TIMING_DVPVO_LOW = 0x01E0 = 480 */
  };
  ...
  {
  case CAMERA_R160x120:
    for (index = 0; index < (sizeof(OV5640_RES_QQVGA) / sizeof(OV5640_RES_QQVGA[0])); index++)
    {
      CAMERA_IO_Write(DeviceAddr, OV5640_RES_QQVGA[index][0], OV5640_RES_QQVGA[index][1]);
    }
    break;

  case CAMERA_R320x240:
    for (index = 0; index < (sizeof(OV5640_RES_QVGA) / sizeof(OV5640_RES_QVGA[0])); index++)
    {
      CAMERA_IO_Write(DeviceAddr, OV5640_RES_QVGA[index][0], OV5640_RES_QVGA[index][1]);
    }
    break;

  case CAMERA_R480x272:
    for (index = 0; index < (sizeof(OV5640_RES_480x272) / sizeof(OV5640_RES_480x272[0])); index++)
    {
      CAMERA_IO_Write(DeviceAddr, OV5640_RES_480x272[index][0], OV5640_RES_480x272[index][1]);
    }
    break;

  case CAMERA_R640x480:
    for (index = 0; index < (sizeof(OV5640_RES_VGA) / sizeof(OV5640_RES_VGA[0])); index++)
    {
      CAMERA_IO_Write(DeviceAddr, OV5640_RES_VGA[index][0], OV5640_RES_VGA[index][1]);
    }
    break;

  default:
    /* Assume 800x480 if not specified */
    for (index = 0; index < (sizeof(OV5640_RES_WVGA) / sizeof(OV5640_RES_WVGA[0])); index++)
    {
      CAMERA_IO_Write(DeviceAddr, OV5640_RES_WVGA[index][0], OV5640_RES_WVGA[index][1]);
    }
    break;
  }
...
/**
 * @brief  Start OV5640 camera streaming.
 * @param  DeviceAddr: Device address on communication Bus.
 * @retval 0 if success, -1 if error
 * @note   Clears the software standby bit to enable streaming
 */
int32_t OV5640_Start(uint16_t DeviceAddr)
{
  /* Read current value of SYSTEM_CTROL0 register */
  uint16_t reg_val = CAMERA_IO_Read(DeviceAddr, 0x3008);

  /* Clear bit 6 (software standby) to start streaming */
  reg_val &= ~(1 << 6);
  CAMERA_IO_Write(DeviceAddr, 0x3008, reg_val);

  CAMERA_Delay(10); /* Small delay for sensor to start */
  return 0;
}

/**
 * @brief  Stop OV5640 camera streaming.
 * @param  DeviceAddr: Device address on communication Bus.
 * @retval 0 if success, -1 if error
 * @note   Sets the software standby bit to stop streaming
 */
int32_t OV5640_Stop(uint16_t DeviceAddr)
{
  /* Read current value of SYSTEM_CTROL0 register */
  uint16_t reg_val = CAMERA_IO_Read(DeviceAddr, 0x3008);

  /* Set bit 6 (software standby) to stop streaming */
  reg_val |= (1 << 6);
  CAMERA_IO_Write(DeviceAddr, 0x3008, reg_val);

  CAMERA_Delay(10); /* Small delay for sensor to stop */
  return 0;
}
