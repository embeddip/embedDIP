...
int camera_init(ImageResolution resolution)
{
    ...
    config.frame_size = (framesize_t)FRAMESIZE_QVGA;
    ...
    if (config.pixel_format == PIXFORMAT_JPEG)
    {
        ...
        if (psramFound())
        {
            ...
            config.fb_count = 2;
            config.grab_mode = CAMERA_GRAB_LATEST;
            Serial.println("[INFO] PSRAM detected. Using higher quality JPEG and double buffers.");
        }
        else
        {
            config.frame_size = (framesize_t)FRAMESIZE_QVGA;
