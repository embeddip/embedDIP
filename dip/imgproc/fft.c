...
void multiply(const Image *img1, const Image *img2, Image *outImg)
{
    ...
    if (!img1 || !img2 || !outImg ||
        isChalsEmpty(img1) || isChalsEmpty(img2))
        return;
    ...
    float *in1 = img1->chals->ch[0];
    float *in2 = img2->chals->ch[0];
...
int fft(const Image *inImg, Image *outImg)
{
    ...
    float *buf0 = (float *)memory_alloc(N * N * 2 * sizeof(float));
    float *buf1 = (float *)memory_alloc(N * N * 2 * sizeof(float));
    outImg->chals->ch[0] = buf0;
    outImg->chals->ch[1] = buf1;
...
void _abs(const Image *fftImg, Image *magImg)
{
    ...
    for (int i = 0; i < size; i++)
    {
        float re = fft[2 * i];
        float im = fft[2 * i + 1];
        mag[i] = log1pf(sqrtf(re * re + im * im));
    }
}
