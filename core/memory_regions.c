...
static embeddip_status_t memory_cache_apply(const void *address, size_t size,
...
{
    if (size == 0u || !memory_alignment_is_power_of_two(alignment) || !memory_region_is_writable(region)) {
        return NULL;
    }
