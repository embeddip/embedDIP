...
#include <fft.h>
/**
 * @brief Applies natural logarithm to each pixel value in a single-channel
 * image.
 *
...
void fftShift(float *data, int width, int height) {
  ...
  for (int y = 0; y < cy; y++) {
    for (int x = 0; x < cx; x++) {
      // Calculate index for top-left (q0), top-right (q1), bottom-left (q2),
      // bottom-right (q3)
      int q0 = 2 * ((y * width) + x);
...
void fourierInv(const Image *inImg, Image *outImg) {
  ...
  for (int k = 0; k < imageN; k++) {
    for (int j = 0; j < imageN; j++) {
      ...
      fourier[2 * j + 1 + k * imageN * 2] =
          (float)fourier2[j * imageN * 2 + k * 2 + 1];
    }
  }
  ...
  for (int i = 0; i < imageN * imageN; i++) {
    outImg->chals->ch[0][i] =
        (float)sqrt(fourier[2 * i] * fourier[2 * i] +
                    fourier[2 * i + 1] * fourier[2 * i + 1]);
  }
}

/**
 * @brief Converts polar coordinates (magnitude and phase) to complex cartesian
 * (real and imaginary).
 *
 ...
 * @param filterType Type of filter to create (low-pass, high-pass, band-pass,
 * etc.).
 ...
 */
void getFilter(Image *maskImg, FrequencyFilterType filterType, float cutoff1,
               float cutoff2) {
  if (!maskImg)
    return;
 ...
 *        ch[0] holds interleaved (Re, Im), ch[1] holds transposed for vertical
 * pass.
 */
 ...
 *        Uses ch[1] as input (transposed buffer), writes to ch[0] as
 * interleaved (Re, Im).
 */
int ifft__(const Image *inImg, Image *outImg) {
  ...
  for (int row = 0; row < N; row++) {
    arm_cfft_f32(&arm_cfft_sR_f32_len256, input + row * N * 2, 1,
                 1); // Inverse FFT
  }
  ...
  for (int row = 0; row < N; row++) {
    arm_cfft_f32(&arm_cfft_sR_f32_len256, buf0 + row * N * 2, 1,
                 1); // Inverse FFT
  }
 ...
 *
 * This function performs element-wise complex multiplication between a
 * Fourier-domain image and a filter mask. The mask can be either a grayscale
 * magnitude mask or a complex-valued mask.
 *
 * @param[in]  fftImg     Complex frequency-domain image (Re, Im interleaved in
 * ch[0]).
 * @param[in]  filterMask Grayscale or complex mask to apply in frequency
 * domain.
...
void ffilter2D(const Image *fftImg, const Image *filterMask, Image *outImg) {
  ...
  if (!fftImg || !filterMask || !outImg || isChalsEmpty(fftImg) ||
      isChalsEmpty(filterMask))
    return;
  ...
  Image *phaseImg =
      createImageWH(fftImg->width, fftImg->height, fftImg->format);
...
void fourier(const Image *inImg, Image *outImg)
{
    ...
    outImg->chals->ch[0] = (float *)memory_alloc(inImg->height * inImg->width *
8); outImg->chals->ch[1] = (float *)memory_alloc(inImg->height * inImg->width *
8);
    ...
    if (isChalsEmpty(inImg))
    {
        ...
        for (int i = 0; i < imageN; i++)
        {
            arm_cfft_f32(&arm_cfft_sR_f32_len256, fourier + imageN * i * 2, 0,
1);
        }
...
+ k * 2]; fourier2[2 * j + 1 + k * imageN * 2] = (float)fourier[j * imageN * 2 +
k * 2 + 1];
            }
        }
        ...
        for (int i = 0; i < imageN; i++)
        {
            arm_cfft_f32(&arm_cfft_sR_f32_len256, fourier2 + imageN * i * 2, 0,
1);
        }
    }
    else
