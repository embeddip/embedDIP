...
{
    const uint8_t bytes_per_pixel = image == NULL ? 0u : image_pixel_size_bytes(image->format, image->depth);
    ...
    return image_view_from_buffer((uint8_t *)image->pixels, image->width, image->height,
                                  image->width * bytes_per_pixel, image->format, image->depth,
                                  EMBEDDIP_MEMORY_REGION_DEFAULT,
                                  EMBEDDIP_BUFFER_CPU_READ | EMBEDDIP_BUFFER_CPU_WRITE, out_view);
}
