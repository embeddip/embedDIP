...
Image::Image() {
  image_ = nullptr; // sadece null pointer
}
...
::Image *Image::raw() noexcept { return image_; }
...
void Image::grayscaleThresholdLocalOtsuTo(Image &out,
...
void Image::grayscaleRegionGrowing(Image &out, const Point *seeds, int numSeeds,
  ...
  ::grayscaleRegionGrowing(raw(), out.raw(), cSeeds.data(), numSeeds,
                           tolerance);
}

void Image::colorRegionGrowing(Image &out, const Point *seeds, int numSeeds,
...
int Image::extractHoughLines(const std::vector<std::vector<int>> &acc,
                             float rhoRes, float thetaRes, int threshold,
                             float rhoMax, std::vector<HoughLine> &lines,
                             int maxLines) const {
  ...
  int count = ::extractLines(rows.data(), numRho, numTheta, rhoRes, thetaRes,
                             threshold, rhoMax, tmp.data(), maxLines);
  lines.assign(tmp.begin(), tmp.begin() + count);
  return count;
}

void Image::drawLine(int x0, int y0, int x1, int y1,
                     uint8_t color) const noexcept {
  ::drawLine(raw(), x0, y0, x1, y1, color);
}

void Image::drawHoughLine(float rho, float theta,
                          uint8_t color) const noexcept {
 ...
 *
 */
void Image::erode(Image &out, const Kernel &kernel,
                  uint8_t iterations) const noexcept {
 ...
 *
 */
void Image::dilate(Image &out, const Kernel &kernel,
                   uint8_t iterations) const noexcept {
 ...
 *
 */
void Image::opening(Image &out, const Kernel &kernel,
                    uint8_t iterations) const noexcept {
 ...
 *
 */
void Image::closing(Image &out, const Kernel &kernel,
                    uint8_t iterations) const noexcept {
...
void Image::convertScaleAbsTo(Image &out, float alpha,
                              float beta) const noexcept {
  ::convertScaleAbs(raw(), out.raw(), alpha, beta);
}

void Image::piecewiseTransform(
    Image &out, const std::vector<uint8_t> &breakpoints,
    const std::vector<uint8_t> &values) const noexcept {
  ::piecewiseTransform(raw(), out.raw(), breakpoints.data(), values.data(),
...
int Image::histEq(Image &out) const { return ::histEq(raw(), out.raw()); }
 ...
 *
 * This function creates a compatible C-style filter kernel and applies
 * the 2D filter to each channel individually using the underlying C
 * function `filter2D_single_channel`. The wrapper supports grayscale and
 * RGB888 images.
 *
...
void Image::filter2D(const std::vector<std::vector<float>> &filter,
  ...
  context.size = size;
  // context.kernel = reinterpret_cast<float(*)[size]>(kernelFlat); // not
  // the best solution.
...
void Image::convertTo() { ::convertTo(raw()); }

// Fourier Transform
void Image::fft(Image &out) const { ::fft(raw(), out.raw()); }

void Image::_abs_(Image &out) const { ::_abs_(raw(), out.raw()); }
...
void Image::createFrequencyMask(FrequencyFilterType type, float cutoff1,
                                float cutoff2) {
 ...
 */
void Image::bitwiseNot(Image &out) const { _not(raw(), out.raw()); }
...
void Image::inRange(Image &mask, const uint8_t lower[3],
                    const uint8_t upper[3]) const {
...
void Image::Canny(Image &outImg, double threshold1, double threshold2,
                  int apertureSize, bool L2gradient) {
  ::Canny(raw(), outImg.raw(), threshold1, threshold2, apertureSize,
          L2gradient);
}
