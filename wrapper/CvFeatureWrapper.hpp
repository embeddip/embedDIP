...
class CvFeatures
{
  public:
    ...
    static embeddip_status_t haarFeature(const CvIntegralU32 &table, int32_t originX,
                                         int32_t originY, const CvHaarRect *rectangles,
                                         uint8_t rectangleCount,
                                         int32_t &responseQ8) noexcept
    {
        return cv_haar_feature_response(&table, originX, originY, rectangles,
                                        rectangleCount, &responseQ8);
    }

    /** @see cv_haar_cascade_eval */
    static embeddip_status_t haarCascade(const CvIntegralU32 &table, int32_t originX,
                                         int32_t originY, const CvHaarCascade &cascade,
    ...
    static embeddip_status_t hogSize(Rectangle roi, const CvHogConfig &config,
    ...
    /** @see cv_hog_extract */
    static embeddip_status_t hog(const ImageView &src, Rectangle roi,
    ...
    static embeddip_status_t linearTopK(const CvLinearClassifier &model,
                                        const float *descriptor, size_t descriptorLength,
                                        size_t topK, CvClassScore *scores,
                                        size_t scoreCapacity, size_t &count) noexcept
    {
        return cv_linear_classifier_topk(&model, descriptor, descriptorLength, topK,
                                         scores, scoreCapacity, &count);
    }
};
